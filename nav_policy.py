...
class NavPolicy2(Policy):
    def pass_through_network(self, x):
        ...
        with tf.control_dependencies([
            tf.assert_equal(tf.shape(angle)[1:], [1]),
            tf.assert_equal(tf.shape(translation)[1:], [2]),
            tf.assert_equal(tf.shape(alpha)[1:], [lidar_size]),
            tf.assert_equal(tf.shape(lidar_)[1:], [2 * lidar_size]),
        ]):
        ...
        with tf.control_dependencies([
            tf.assert_equal(tf.shape(hidden_map)[1:], [height, width, 1]),
            tf.assert_equal(tf.shape(theta)[1:], [2, 3]),
        ]):
        ...
        for lidar_index, (lidar_params, alpha) in enumerate(zip(
                tf.unstack(lidar, axis=2),
                tf.unstack(alpha, axis=1)
        )):
